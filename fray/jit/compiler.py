...
from .template import serialize_arg_defs
...
def parse_cuda_version(version: str) -> Version:
    return Version(version)


def get_gencode_flag(major: int, minor: int) -> str:
    ...
    if arch_code in arch_map:
        if env_flag("FRAY_JIT_DEBUG"):
            print(
                f"Detected local GPU with architecture sm_{arch_code}, "
                "adding specific gencode flag."
            )
        return arch_map[arch_code]

    print(
        f"Warning: unsupported GPU architecture sm_{arch_code} detected. "
...
@functools.lru_cache(maxsize=None)
def get_extra_include_dirs() -> Tuple[str, ...]:
    base_dir = os.path.normpath(f"{os.path.dirname(os.path.abspath(__file__))}/../..")
    candidate_dirs = (
        os.path.join(base_dir, "third-party", "cutlass", "include"),
        os.path.join(base_dir, "third-party", "flashinfer", "include"),
        os.path.join(base_dir, "third-party", "ThunderKittens", "include"),
    )
...
def get_fray_version() -> str:
    ...
    assert os.path.exists(base_include_dir), (
        f"Cannot find Fray include directory {base_include_dir}"
    )
    ...
    for root, _, files in os.walk(jit_dir):
        for file in files:
            if file.endswith(".py"):
                version_files.append(os.path.join(root, file))

    for file_path in sorted(version_files):
        md5.update(os.path.relpath(file_path, start=jit_dir).encode("utf-8"))
...
def get_nvcc_compiler() -> Tuple[str, str]:
    paths = []
    if os.getenv("FRAY_NVCC_COMPILER"):
        paths.append(os.getenv("FRAY_NVCC_COMPILER"))
    ...
    least_version_required = Version("12.3")
        ...
        if os.path.exists(path):
            try:
                ...
                if match:
                    ...
                    if parse_cuda_version(version) >= least_version_required:
                        return path, version
                    else:
                        print(
    ...
    raise RuntimeError(
        "Cannot find any available NVCC compiler with version >= 12.3. "
...
def get_default_user_dir():
    if "FRAY_CACHE_DIR" in os.environ:
        path = os.getenv("FRAY_CACHE_DIR")
...
def build(name: str, arg_defs: tuple, code: str) -> Runtime:
    ...
    common_flags.append(get_gencode_flag(major, minor))

    # Platform-specific flags
    if IS_WINDOWS:
        # 确保 common_flags 里只有一个 -std=c++17
        nvcc_flags = [
            *common_flags,
            "-shared",
        ...
        nvcc_flags = [
            *common_flags,
            "-shared",
            "--ptxas-options=--register-usage-level=10"
            + (",--verbose" if "FRAY_PTXAS_VERBOSE" in os.environ else ""),
            "--diag-suppress=177,174,940",
        ]
        cxx_flags = ["-fPIC", "-O3", "-Wno-deprecated-declarations", "-Wno-abi"]
        flags = [*nvcc_flags, f"--compiler-options={','.join(cxx_flags)}"]

    include_dirs = [get_jit_include_dir(), *get_extra_include_dirs()]

    # Build signature
    nvcc_path, nvcc_version = get_nvcc_compiler()
    enable_sass_opt = parse_cuda_version(nvcc_version) <= Version(
        "12.8"
    ) and not env_flag("FRAY_DISABLE_FFMA_INTERLEAVE")
    signature = f"{name}$${get_fray_version()}$${code}$${(nvcc_path, nvcc_version)}$${flags}$${enable_sass_opt}$${IS_WINDOWS}"
    ...
    put(args_path, serialize_arg_defs(arg_defs))
    put(src_path, code)

    # Compile into a temporary lib file
    lib_path = os.path.join(path, f"kernel{lib_ext}")
    tmp_lib_path = os.path.join(
        make_tmp_dir(), f"nvcc.tmp.{str(uuid.uuid4())}.{hash_to_hex(lib_path)}{lib_ext}"
    )
    ...
    if env_flag("FRAY_JIT_DEBUG") or env_flag("FRAYJIT_PRINT_NVCC_COMMAND"):
