        ...
        if constexpr (test::template valid<S, NUM_WORKERS>::value) {
            ...
            for (int dev_idx = 0; dev_idx < (test::single_run ? 1 : NUM_DEVICES); ++dev_idx) {
                ...
                cudaFuncSetAttribute(
                    p2s_global_wrapper_1d<test, S, NUM_WORKERS, typename shared_layout::PGL, args...>,
                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                    kittens::MAX_SHARED_MEMORY
                );
                p2s_global_wrapper_1d<test, S, NUM_WORKERS, typename shared_layout::PGL, args...><<<1, NUM_WORKERS*32, kittens::MAX_SHARED_MEMORY>>>(*shared_layout::input_pgl, *shared_layout::output_pgl, dev_idx);
            }
    ...
    __device__ static void device_func(const PGL &input, const PGL &output, const int dev_idx) {
        ...
        for (int i = 0; i < num_iters; i++) {
            if constexpr (OP == kittens::ReduceOp::ADD) {
                ...
                kittens::all_reduce_min(s_vec, input, dev_idx, {i});
            } else if constexpr (OP == kittens::ReduceOp::MAX) {
                kittens::all_reduce_max(s_vec, input, dev_idx, {i});
            }
            kittens::store(output[dev_idx], s_vec, {i});
        }
    }
};

template<typename T>
struct p2s_atomic_add_test {
    using dtype = T;
    using _valid_type = std::bool_constant<std::is_same_v<T, float> || std::is_same_v<T, kittens::bf16> || std::is_same_v<T, kittens::half>>;
    template<int S, int NW> using _valid_dims = std::bool_constant<NW == 1 && S <= 64>;
    template<int S, int NW> using valid = std::bool_constant<_valid_type::value && _valid_dims<S, NW>::value>;
...
using p2s_sweep_size_1d = mg_loop_s<p2s_test_wrapper_1d, test, NUM_DEVICES, MAX_S, NUM_WORKERS, MAX_S, args...>;

template<typename test, int NUM_DEVICES, int MAX_S=8, typename... args>
using p2s_sweep_size_1d_warp = p2s_sweep_size_1d<test, NUM_DEVICES, MAX_S, 1, args...>;
...
struct p2s_sweep_size_1d_warp_axes_ops {
    static void run(test_data &results) {    
        ...
        p2s_sweep_size_1d_warp_layouts<p2s_all_reduce_test<T, kittens::ReduceOp::ADD>, NUM_DEVICES, MAX_S>::run(results);
        if constexpr (!std::is_same<T, float>::value) {
            p2s_sweep_size_1d_warp_layouts<p2s_all_reduce_test<T, kittens::ReduceOp::MIN>, NUM_DEVICES, MAX_S>::run(results);
            p2s_sweep_size_1d_warp_layouts<p2s_all_reduce_test<T, kittens::ReduceOp::MAX>, NUM_DEVICES, MAX_S>::run(results);
        }

        p2s_sweep_size_1d_warp_layouts<p2s_atomic_add_test<T>, NUM_DEVICES, MAX_S>::run(results);
        p2s_sweep_size_1d_warp_layouts<p2s_broadcast_test<T>, NUM_DEVICES, MAX_S>::run(results);
...
void warp::memory::vec::pgl_to_shared::tests(test_data &results) {
    std::cout << "\n ----- Starting ops/warp/memory/vec/pgl_to_shared tests! -----\n" << std::endl;
    ...
    if (check_multi_gpus()) {
        p2s_sweep_size_1d_warp_axes_ops<float, NUM_GPUS, SIZE>::run(results);
        p2s_sweep_size_1d_warp_axes_ops<kittens::bf16, NUM_GPUS, SIZE>::run(results);
        p2s_sweep_size_1d_warp_axes_ops<kittens::half, NUM_GPUS, SIZE>::run(results);
    }
}
