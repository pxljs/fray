...
struct test_mma_ABt_fp16_fp8 {
    ...
    using valid = std::bool_constant<
        ...
        (W%2 == 0)
    >; // this is warp-level
    static inline const std::string test_identifier = "wgmma_mma_ABt_fp16_fp8";
...
struct reg_test_mma_ABt_fp16_fp8 {
    ...
    using valid = std::bool_constant<
        ...
        (W%2 == 0)
    >; // this is warp-level
    static inline const std::string test_identifier = "wgmma_reg_mma_ABt_fp16_fp8";
    template<int H, int W, int NW, gl_t GTL_A, gl_t GTL_B, gl_t GTL_C, typename _K>
    __host__ static void host_func(const std::vector<float> &i_ref, std::vector<float> &o_ref) {
        constexpr int K = _K::value;
        for(int i = 0; i < H*16; i++) {
...
void group::wgmma::mma_fp16_fp8::tests(test_data &results) {
    std::cout << "\n ----- Starting ops/warpgroup/wgmma/mma_fp16_fp8 tests! -----\n" << std::endl;
