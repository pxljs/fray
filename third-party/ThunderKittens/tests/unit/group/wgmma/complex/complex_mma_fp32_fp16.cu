...
struct test_cmplx_mma_AB_fp32_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H==4 && (2*W*H+W*K::value+H*K::value)<=256>;
    static inline const std::string test_identifier = "complex_wgmma_mma_AB_fp32_fp16";
    ...
    __device__ static void device_func(const CGL1 &A, const CGL2 &B, const CGL3 &C) {
        ...
        kittens::cst_hf<16*H, 16*K> &a = al.allocate<kittens::cst_hf<16*H, 16*K>>();
        kittens::cst_hf<16*K, 16*W> &b = al.allocate<kittens::cst_hf<16*K, 16*W>>();
...
struct test_cmplx_mma_ABt_fp32_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H==4 && (2*W*H+W*K::value+H*K::value)<=256>; // this is warp-level
    static inline const std::string test_identifier = "complex_wgmma_mma_ABt_fp32_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
    ...
    __device__ static void device_func(const GTL_A &a_input, const GTL_B &b_input, GTL_C &c_output) {
        ...
        kittens::tma_swizzle_allocator al((int*)&__shm[0]); 
        kittens::cst_hf<16*H, 16*K> &a = al.allocate<kittens::cst_hf<16*H, 16*K>>();
        kittens::cst_hf<16*W, 16*K> &b = al.allocate<kittens::cst_hf<16*W, 16*K>>();
        kittens::crt_fl<16, 16*W> c;
...
struct test_cmplx_mma_AtB_fp32_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H==4 && (2*W*H+W*K::value+H*K::value)<=256>; // this is warp-level
    static inline const std::string test_identifier = "complex_wgmma_mma_AtB_fp32_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
     __host__ static void host_func(const std::vector<float> &re_i_ref, const std::vector<float> &im_i_ref,
        std::vector<float> &re_o_ref, std::vector<float> &im_o_ref) {
        constexpr int K = _K::value;
    ...
    __device__ static void device_func(const GTL_A &a_input, const GTL_B &b_input, GTL_C &c_output) {
        ...
        extern __shared__ kittens::alignment_dummy __shm[]; // this is the CUDA shared memory
        kittens::tma_swizzle_allocator al((int*)&__shm[0]); 
        kittens::cst_hf<16*K, 16*H> &a = al.allocate<kittens::cst_hf<16*K, 16*H>>();
        kittens::cst_hf<16*K, 16*W> &b = al.allocate<kittens::cst_hf<16*K, 16*W>>();
        kittens::crt_fl<16, 16*W> c;
        kittens::warpgroup::load_async(a, a_input, {});
        kittens::warpgroup::load_async(b, b_input, {});
...
struct test_cmplx_mma_AtBt_fp32_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H==4 && (2*W*H+W*K::value+H*K::value)<=256>; // this is warp-level
    static inline const std::string test_identifier = "complex_wgmma_mma_AtBt_fp32_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
    __host__ static void host_func(const std::vector<float> &re_i_ref, const std::vector<float> &im_i_ref,
        std::vector<float> &re_o_ref, std::vector<float> &im_o_ref) {
        constexpr int K = _K::value;
...
struct reg_cmplx_test_mma_AB_fp32_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H%4==0 && (2*W*H+W*K::value+H*K::value)<=256>;
    static inline const std::string test_identifier = "complex_wgmma_reg_mma_AB_fp32_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
     __host__ static void host_func(const std::vector<float> &re_i_ref, const std::vector<float> &im_i_ref,
        std::vector<float> &re_o_ref, std::vector<float> &im_o_ref) {
        constexpr int K = _K::value;
...
struct reg_cmplx_test_mma_ABt_fp32_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H%4==0 && (2*W*H+W*K::value+H*K::value)<=256>; // this is warp-level
    static inline const std::string test_identifier = "complex_wgmma_reg_mma_ABt_fp32_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
    __host__ static void host_func(const std::vector<float> &re_i_ref, const std::vector<float> &im_i_ref,
        std::vector<float> &re_o_ref, std::vector<float> &im_o_ref) {
        constexpr int K = _K::value;
...
struct mma_wrapper_2d {
    static void run(test_data& results) {
        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS, _K, args...>::value) {
            ...
            initialize<half, initializers::RANDOM, 24>(&d_im_i, &d_im_o, im_i_ref, im_o_ref);
...
void group::wgmma::complex::complex_mma_fp32_fp16::tests(test_data &results) {
    std::cout << "\n ----- Starting ops/warpgroup/wgmma/complex/complex_mma_fp32_fp16 tests! -----\n" << std::endl;
    ...
    mma_sweep_width_warpgroup<test_cmplx_mma_AB_fp32_fp16,   SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_ABt_fp32_fp16,  SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtB_fp32_fp16,  SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtBt_fp32_fp16, SIZE, I1_t>::run(results);

    mma_sweep_width_warpgroup<test_cmplx_mma_AB_fp32_fp16,   SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_ABt_fp32_fp16,  SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtB_fp32_fp16,  SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtBt_fp32_fp16, SIZE, I3_t>::run(results);

    mma_sweep_width_warpgroup<test_cmplx_mma_AB_fp32_fp16,   SIZE, I5_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_ABt_fp32_fp16,  SIZE, I5_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtB_fp32_fp16,  SIZE, I5_t>::run(results); 
    mma_sweep_width_warpgroup<test_cmplx_mma_AtBt_fp32_fp16, SIZE, I5_t>::run(results);

    // register+shared->reg
    mma_sweep_width_warpgroup<reg_cmplx_test_mma_AB_fp32_fp16,  SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup<reg_cmplx_test_mma_ABt_fp32_fp16, SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_AB_fp32_fp16,  SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_ABt_fp32_fp16, SIZE, I1_t>::run(results);

    mma_sweep_width_warpgroup<reg_cmplx_test_mma_AB_fp32_fp16,  SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup<reg_cmplx_test_mma_ABt_fp32_fp16, SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_AB_fp32_fp16,  SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_ABt_fp32_fp16, SIZE, I3_t>::run(results);

    mma_sweep_width_warpgroup<reg_cmplx_test_mma_AB_fp32_fp16,  SIZE, I5_t>::run(results);
    mma_sweep_width_warpgroup<reg_cmplx_test_mma_ABt_fp32_fp16, SIZE, I5_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_AB_fp32_fp16,  SIZE, I5_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_ABt_fp32_fp16, SIZE, I5_t>::run(results);
}
