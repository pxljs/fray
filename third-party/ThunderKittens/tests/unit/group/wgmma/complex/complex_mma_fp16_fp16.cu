...
struct test_cmplx_mma_AB_fp16_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H==4 && (2*W*H+W*K::value+H*K::value)<=256>;
    static inline const std::string test_identifier = "complex_wgmma_mma_AB_fp16_fp16";
...
struct test_cmplx_mma_ABt_fp16_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H==4 && (2*W*H+W*K::value+H*K::value)<=256>; // this is warp-level
    static inline const std::string test_identifier = "complex_wgmma_mma_ABt_fp16_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
...
struct test_cmplx_mma_AtB_fp16_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H==4 && (2*W*H+W*K::value+H*K::value)<=256>; // this is warp-level
    static inline const std::string test_identifier = "complex_wgmma_mma_AtB_fp16_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
     __host__ static void host_func(const std::vector<float> &re_i_ref, const std::vector<float> &im_i_ref,
        std::vector<float> &re_o_ref, std::vector<float> &im_o_ref) {
        constexpr int K = _K::value;
...
struct test_cmplx_mma_AtBt_fp16_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H==4 && (2*W*H+W*K::value+H*K::value)<=256>; // this is warp-level
    static inline const std::string test_identifier = "complex_wgmma_mma_AtBt_fp16_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
    __host__ static void host_func(const std::vector<float> &re_i_ref, const std::vector<float> &im_i_ref,
        std::vector<float> &re_o_ref, std::vector<float> &im_o_ref) {
        constexpr int K = _K::value;
...
struct reg_cmplx_test_mma_AB_fp16_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H%4==0 && (2*W*H+W*K::value+H*K::value)<=256>;
    static inline const std::string test_identifier = "complex_wgmma_reg_mma_AB_fp16_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
     __host__ static void host_func(const std::vector<float> &re_i_ref, const std::vector<float> &im_i_ref,
        std::vector<float> &re_o_ref, std::vector<float> &im_o_ref) {
        constexpr int K = _K::value;
...
struct reg_cmplx_test_mma_ABt_fp16_fp16 {
    template<int H, int W, int NW, typename K>
    using valid = std::bool_constant<NW == 4 && H%4==0 && (2*W*H+W*K::value+H*K::value)<=256>; // this is warp-level
    static inline const std::string test_identifier = "complex_wgmma_reg_mma_ABt_fp16_fp16";
    template<int H, int W, int NW, typename GTL_A, typename GTL_B, typename GTL_C, typename _K>
    __host__ static void host_func(const std::vector<float> &re_i_ref, const std::vector<float> &im_i_ref,
        std::vector<float> &re_o_ref, std::vector<float> &im_o_ref) {
        constexpr int K = _K::value;
...
void group::wgmma::complex::complex_mma_fp16_fp16::tests(test_data &results) {
    std::cout << "\n ----- Starting ops/warpgroup/wgmma/complex/complex_mma_fp16_fp16 tests! -----\n" << std::endl;
    ...
    mma_sweep_width_warpgroup<test_cmplx_mma_AB_fp16_fp16,   SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_ABt_fp16_fp16,  SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtB_fp16_fp16,  SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtBt_fp16_fp16, SIZE, I1_t>::run(results);

    mma_sweep_width_warpgroup<test_cmplx_mma_AB_fp16_fp16,   SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_ABt_fp16_fp16,  SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtB_fp16_fp16,  SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtBt_fp16_fp16, SIZE, I3_t>::run(results);

    mma_sweep_width_warpgroup<test_cmplx_mma_AB_fp16_fp16,   SIZE, I5_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_ABt_fp16_fp16,  SIZE, I5_t>::run(results);
    mma_sweep_width_warpgroup<test_cmplx_mma_AtB_fp16_fp16,  SIZE, I5_t>::run(results); 
    mma_sweep_width_warpgroup<test_cmplx_mma_AtBt_fp16_fp16, SIZE, I5_t>::run(results);

    // register+shared->reg
    mma_sweep_width_warpgroup<reg_cmplx_test_mma_AB_fp16_fp16,  SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup<reg_cmplx_test_mma_ABt_fp16_fp16, SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_AB_fp16_fp16,  SIZE, I1_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_ABt_fp16_fp16, SIZE, I1_t>::run(results);

    mma_sweep_width_warpgroup<reg_cmplx_test_mma_AB_fp16_fp16,  SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup<reg_cmplx_test_mma_ABt_fp16_fp16, SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_AB_fp16_fp16,  SIZE, I3_t>::run(results);
    mma_sweep_width_warpgroup_doubleheight<reg_cmplx_test_mma_ABt_fp16_fp16, SIZE, I3_t>::run(results);

    mma_sweep_width_warpgroup<reg_cmplx_test_mma_AB_fp16_fp16,  SIZE, I5_t>::run(results);
