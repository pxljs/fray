        ...
        if constexpr (test::template valid<S, NUM_WORKERS>::value) {
            ...
            for (int dev_idx = 0; dev_idx < (test::single_run ? 1 : NUM_DEVICES); ++dev_idx) {
                ...
                cudaFuncSetAttribute(
                    group_vec_p2r_global_wrapper_1d<test, S, NUM_WORKERS, typename shared_layout::PGL, args...>,
                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                    kittens::MAX_SHARED_MEMORY
                );
                group_vec_p2r_global_wrapper_1d<test, S, NUM_WORKERS, typename shared_layout::PGL, args...><<<1, NUM_WORKERS*32>>>(*shared_layout::input_pgl, *shared_layout::output_pgl, dev_idx);
            }
    ...
    __device__ static void device_func(const PGL &input, const PGL &output, const int dev_idx) {
        ...
        for (int i = 0; i < num_iters; i++) {
            if constexpr (OP == kittens::ReduceOp::ADD) {
                ...
                G::template all_reduce_min(reg_vec, input, dev_idx, {i});
            } else if constexpr (OP == kittens::ReduceOp::MAX) {
                G::template all_reduce_max(reg_vec, input, dev_idx, {i});
            }
...
template<typename T>
struct group_vec_p2r_atomic_add_test {
    using dtype = T;
    using _valid_type = std::bool_constant<std::is_same_v<T, float> || std::is_same_v<T, kittens::bf16> || std::is_same_v<T, kittens::half>>;
    template<int S, int NW> using _valid_dims = std::bool_constant<S <= 64>;
    template<int S, int NW> using valid = std::bool_constant<_valid_type::value && _valid_dims<S, NW>::value>;
    static inline const bool single_run = false;
    static inline const std::string test_identifier = std::is_same_v<T, kittens::bf16> ? "group_vec_p2r_atomic_add=bf16" :
                                                      std::is_same_v<T, kittens::half> ? "group_vec_p2r_atomic_add=half" :
                                                                                         "group_vec_p2r_atomic_add=float";
    template<int S, int NW, kittens::ducks::pgl::all PGL, typename... args>
    __host__ static void host_func(const std::vector<std::vector<float>> &i_ref, std::vector<std::vector<float>> &o_ref) {
        // each vector represents a GPU device holding the data
        for (int dev_idx = 0; dev_idx < i_ref.size(); ++dev_idx) {
...
template<typename T>
struct group_vec_p2r_broadcast_test {
    using dtype = T;
    using _valid_type = std::bool_constant<std::is_same_v<T, float> || std::is_same_v<T, kittens::bf16> || std::is_same_v<T, kittens::half>>;
    template<int S, int NW> using _valid_dims = std::bool_constant<S <= 64>;
    template<int S, int NW> using valid = std::bool_constant<_valid_type::value && _valid_dims<S, NW>::value>;
    static inline const bool single_run = true;
    static inline const std::string test_identifier = std::is_same_v<T, kittens::bf16> ? "group_vec_p2r_broadcast=bf16" :
                                                      std::is_same_v<T, kittens::half> ? "group_vec_p2r_broadcast=half" :
                                                                                         "group_vec_p2r_broadcast=float";
    template<int S, int NW, kittens::ducks::pgl::all PGL, typename... args>
    __host__ static void host_func(const std::vector<std::vector<float>> &i_ref, std::vector<std::vector<float>> &o_ref) {
        // each vector represents a GPU device holding the data
        for (int dev_idx = 0; dev_idx < i_ref.size(); ++dev_idx) {
...
struct group_vec_p2r_sweep_size_1d_warp_axes_ops {
    static void run(test_data &results) {    
        ...
        group_vec_p2r_sweep_size_1d_warp_layouts_nw<group_vec_p2r_atomic_add_test<T>, NUM_DEVICES, MAX_S>::run(results);
        group_vec_p2r_sweep_size_1d_warp_layouts_nw<group_vec_p2r_broadcast_test<T>, NUM_DEVICES, MAX_S>::run(results);
