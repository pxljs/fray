        ...
        if constexpr (test::template valid<H, W, NUM_WORKERS>::value) {
            ...
            for (int dev_idx = 0; dev_idx < (test::single_run ? 1 : NUM_DEVICES); ++dev_idx) {
                ...
                cudaFuncSetAttribute(
                    group_p2r_global_wrapper_2d<test, H, W, NUM_WORKERS, typename shared_layout::PGL, axis, args...>,
                    cudaFuncAttributeMaxDynamicSharedMemorySize,
                    kittens::MAX_SHARED_MEMORY
                );
                group_p2r_global_wrapper_2d<test, H, W, NUM_WORKERS, typename shared_layout::PGL, axis, args...><<<1, NUM_WORKERS*32>>>(*shared_layout::input_pgl, *shared_layout::output_pgl, dev_idx);
            }
...
template<typename T>
struct group_p2r_atomic_add_test {
    using dtype = T;
    using _valid_type = std::bool_constant<std::is_same_v<T, float> || std::is_same_v<T, kittens::bf16> || std::is_same_v<T, kittens::half>>;
    template<int H, int W, int NW> using _valid_dims = std::bool_constant<H%NW==0 && W*H<=64>; // this is group-level
    template<int H, int W, int NW> using valid = std::bool_constant<_valid_type::value && _valid_dims<H, W, NW>::value>;
    static inline const bool single_run = false;
    static inline const std::string test_identifier = std::is_same_v<T, kittens::bf16> ? "group_p2r_atomic_add=bf16" :
                                                      std::is_same_v<T, kittens::half> ? "group_p2r_atomic_add=half" :
                                                                                         "group_p2r_atomic_add=float";
    template<int H, int W, int NW, kittens::ducks::pgl::all PGL, typename axis>
    __host__ static void host_func(const std::vector<std::vector<float>> &i_ref, std::vector<std::vector<float>> &o_ref) {
        // each vector represents a GPU device holding the data
        for (int dev_idx = 0; dev_idx < i_ref.size(); ++dev_idx) {
...
template<typename T>
struct group_p2r_broadcast_test {
    using dtype = T;
    using _valid_type = std::bool_constant<std::is_same_v<T, float> || std::is_same_v<T, kittens::bf16> || std::is_same_v<T, kittens::half>>;
    template<int H, int W, int NW> using _valid_dims = std::bool_constant<H%NW==0 && W*H<=64>; // this is group-level
    template<int H, int W, int NW> using valid = std::bool_constant<_valid_type::value && _valid_dims<H, W, NW>::value>;
    static inline const bool single_run = true;
    static inline const std::string test_identifier = std::is_same_v<T, kittens::bf16> ? "group_p2r_broadcast=bf16" :
                                                      std::is_same_v<T, kittens::half> ? "group_p2r_broadcast=half" :
                                                                                         "group_p2r_broadcast=float";
    template<int H, int W, int NW, kittens::ducks::pgl::all PGL, typename axis>
    __host__ static void host_func(const std::vector<std::vector<float>> &i_ref, std::vector<std::vector<float>> &o_ref) {
        // each vector represents a GPU device holding the data
        for (int dev_idx = 0; dev_idx < i_ref.size(); ++dev_idx) {
...
struct group_p2r_sweep_size_2d_group_axes_ops {
    static void run(test_data &results) {    
        ...
        group_p2r_sweep_size_2d_group_axes_nw<group_p2r_atomic_add_test<T>, NUM_DEVICES, MAX_H, MAX_W>::run(results);
        group_p2r_sweep_size_2d_group_axes_nw<group_p2r_broadcast_test<T>, NUM_DEVICES, MAX_H, MAX_W>::run(results);
