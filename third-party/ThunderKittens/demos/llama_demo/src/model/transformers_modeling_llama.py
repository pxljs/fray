...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        # attn_output_tk = self.o.transpose(1, 2).contiguous()
        
        # # breakpoint()
        # if pad_len is not None:
        #     attn_output_tk = attn_output_tk[:, :q_len, :, :]

        # # breakpoint()
        # diff = torch.abs(attn_output - attn_output_tk)
        # if q_len > 1 and self.layer_idx == 0:
        #     print(f"Max diff: {diff.max()}")

        # # if self.layer_idx == 0 and q_len == 1:
        # #         print(f"{q_len}, {attn_output[0,0,0,:4]}")

        # if q_len > 1: # prefill
        #     attn_output = attn_output_tk
...
class LlamaTKAttention(LlamaAttention):
    """
    Llama TK attention module. This module inherits from `LlamaAttention` as the weights of the module stays
    ...
    def forward(
        ...
        if isinstance(past_key_value, StaticCache):
            raise ValueError(
                "`static` cache implementation is not compatible with `attn_implementation==tk_attention` "
                "make sure to use `sdpa` in the mean time, and open an issue at https://github.com/huggingface/transformers"
            )
        ...
        if q_len > 1: # Prefill with TK
            # Pad the input if it is not a multiple of 192
            pad_len = None 
            if q_len % 192 != 0:
                pad_len = 192 - q_len % 192
                query_states = F.pad(query_states, (0, 0, 0, pad_len))
