...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin, PushToHubMixin, PeftAdapterMixin):
    ...
    # TK Attention support
    _supports_tk_attention = True # default
    ...
    def _autoset_attn_implementation(
        ...
        if hasattr(config, "_attn_implementation_internal") and config._attn_implementation_internal is not None:
            ...
            if config._attn_implementation not in ["eager", "sdpa", "flash_attention_2", "tk_attention"]:
                message = f'Specified `attn_implementation="{config._attn_implementation}"` is not supported. The only possible arguments are `attn_implementation="eager"` (manual attention implementation)'
                if cls._supports_flash_attn_2:
                    message += ', `"attn_implementation=flash_attention_2"` (implementation using flash attention 2)'
                if cls._supports_sdpa:
                    message += ', `"attn_implementation=sdpa"` (implementation using torch.nn.functional.scaled_dot_product_attention)'
                if cls._supports_tk_attention:
                    message += ', `"attn_implementation=tk_attention"` (implementation using torch.nn.functional.multi_head_attention'
        ...
        if use_tk_attention:
            config._attn_implementation = "tk_attention"
        ...
        elif config._attn_implementation == "tk_attention":
            cls._check_and_enable_tk_attention(
                config,
                torch_dtype=torch_dtype,
                device_map=device_map,
                hard_check_only=False,
    ...
    @classmethod
    def _check_and_enable_tk_attention(
        cls,
        config,
        torch_dtype: Optional[torch.dtype] = None,
        device_map: Optional[Union[str, Dict[str, int]]] = None,
        ...
        """
        Checks the availability of TK Attention and compatibility with the current model.

        If all checks pass and `hard_check_only` is False, the method will set the config attribute `attn_implementation` to "flash_attention_2" so that the model can initialize the correct attention module.
        """
        if not cls._supports_tk_attention:
            raise ValueError(
                f"{cls.__name__} does not support TK yet. Please request to add support where"
                f" the model is hosted, on its model hub page: https://huggingface.co/{config._name_or_path}/discussions/new"
                " or in the Transformers GitHub repo: https://github.com/huggingface/transformers/issues/new"
            )
        ...
        if torch_dtype is None:
            logger.warning_once(
                "You are attempting to use TK without specifying a torch dtype. This might lead to unexpected behaviour"
            )
        elif torch_dtype is not None and torch_dtype not in [torch.float16, torch.bfloat16]:
            logger.warning_once(
                "TK only supports torch.float16 and torch.bfloat16 dtypes, but"
                f" the current dype in {cls.__name__} is {torch_dtype}. You should run training or inference using Automatic Mixed-Precision via the `with torch.autocast(device_type='torch_device'):` decorator,"
                ' or load the model with the `torch_dtype` argument. Example: `model = AutoModel.from_pretrained("openai/whisper-tiny", attn_implementation="tk_attention", torch_dtype=torch.float16)`'
            )
        ...
        if check_device_map and device_map is None and torch.empty(0).device.type != "cuda":
            ...
            raise ValueError(
                "You are attempting to use TK with a model dispatched on CPU or disk. This is not supported. Please make sure to "
                "initialise the model on a GPU by passing a device_map that contains only GPU devices as keys."
            )
        if not hard_check_only:
            config._attn_implementation = "tk_attention"
