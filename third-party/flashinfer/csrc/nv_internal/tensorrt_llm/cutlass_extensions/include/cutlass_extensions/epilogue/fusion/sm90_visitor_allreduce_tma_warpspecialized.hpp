...
template <int Stages, typename EpilogueTile, typename ElementT, typename StrideMNL,
          typename SmemLayoutAtom, FloatRoundStyle RoundStyle, typename CopyOpR2S,
          typename TileShape, typename SystemBarrier_, bool OneShot>
struct Sm90AuxAllReduce {
  ...
  using SmemLayoutTma =
      decltype(tile_to_shape(SmemLayoutAtom{}, SmemShapeTma{},
  ...
  struct SharedStorage {
    alignas(cutlass::detail::alignment_for_swizzle(SmemLayout{}))
        array_aligned<ElementT, size(SmemLayout{})> smem_aux;
  };
  ...
  struct Params {
    using TMA_Aux =
        decltype(make_tma_copy(get_TMA_store_op(),
                               make_tensor(static_cast<ElementT*>(nullptr),
  ...
  static constexpr Params to_underlying_arguments(ProblemShape const& problem_shape,
    ...
    typename Params::TMA_Aux tma_store_aux =
        make_tma_copy(get_TMA_store_op(), tensor_aux, SmemLayoutTma{});
    ...
    return {tma_store_aux, args.multicast_ptr_aux, args.dAux, tile_layout, args.barrier_params,
  ...
  template <int NumThreads,  // number of threads that cooperatively execute this EVT node function
            class RTensor, class TiledR2S, class STensorR2S, class STensorS2G, class GTensorS2G,
            class ProblemShapeMNL, class TileCoordMNL>
  struct ConsumerStoreCallbacks : EmptyConsumerStoreCallbacks {
    CUTLASS_DEVICE
    ConsumerStoreCallbacks(RTensor&& tC_rAux, TiledR2S tiled_r2s, STensorR2S&& tRS_sAux,
                           STensorS2G&& bSG_sAux, GTensorS2G&& bSG_gAux, Params const* params_ptr,
                           ProblemShapeMNL problem_shape_mnl, TileCoordMNL tile_coord_mnl,
    ...
    CUTLASS_DEVICE auto visit(Array<ElementAccumulator, FragmentSize> const& frg_acc, int epi_v,
                              int epi_m, int epi_n,
    ...
    CUTLASS_DEVICE void postreduce(int epi_m, int epi_n, int store_iteration,
                                   bool issue_smem_store) {
    ...
    CUTLASS_DEVICE void tma_store(int epi_m, int epi_n, int store_iteration, bool issue_tma_store) {
      if (issue_tma_store && thread_idx == 0) {
        // Issue the TMA store
        int store_pipe_index = store_iteration % Stages;
        copy(params_ptr->tma_store_aux, bSG_sAux(_, _, _, store_pipe_index),
             bSG_gAux(_, _, _, epi_m, epi_n));
      }
    ...
    CUTLASS_DEVICE void end() {
      ...
      SystemBarrier::arrive_inc(params_ptr->barrier_params, thread_idx, tile_idx, params_ptr->rank,
  ...
  CUTLASS_DEVICE auto get_consumer_store_callbacks(ConsumerStoreArgs<Args...> const& args) {
    ...
    Tensor gAux =
        local_tile(mAux, take<0, 2>(args.tile_shape_mnk), tile_coord_mnl);  // (CTA_M,CTA_N)
    ...
    auto tiled_r2s = conditional_return<ReferenceSrc>(
        make_tiled_copy_S(Copy_Atom<CopyOpR2S, ElementT>{}, args.tiled_copy),
        make_tiled_copy_D(Copy_Atom<CopyOpR2S, ElementT>{}, args.tiled_copy));
    auto tRS_sAux =
        tiled_r2s.get_slice(args.thread_idx).partition_D(sAux_epi);  // (R2S,R2S_M,R2S_N,PIPE)
    ...
    return ConsumerStoreCallbacks<NumThreads, decltype(tC_rAux), decltype(tiled_r2s),
                                  decltype(tRS_sAux), decltype(bSG_sAux), decltype(bSG_gAux),
                                  decltype(problem_shape_mnl), decltype(tile_coord_mnl)>(
        cute::move(tC_rAux), tiled_r2s, cute::move(tRS_sAux), cute::move(bSG_sAux),
        cute::move(bSG_gAux), params_ptr, problem_shape_mnl, tile_coord_mnl, args.thread_idx);
  }
};

// D = AllReduce(activation(alpha * acc + beta * C))
template <bool IsOneShot_, class SystemBarrier_, class GmemLayoutTagAux_, class ElementAux_,
          class ElementCompute_, class ElementSource_ = ElementAux_,
...
template <int StagesD, bool IsOneShot, class SystemBarrier, class GmemLayoutTagOutput,
...
using Sm90LinearCombAuxAllReduce = Sm90EVT<
    Sm90AuxAllReduce<StagesD, EpilogueTile, ElementOutput,
                     cutlass::gemm::TagToStrideC_t<GmemLayoutTagOutput>, SmemLayoutAtom, RoundStyle,
                     CopyOpR2S, CtaTileShapeMNK, SystemBarrier, IsOneShot>,  // Aux AR
    Sm90LinearCombination<ElementOutput, ElementCompute, ElementSource, ElementScalar,
                          RoundStyle>  // beta * C +
                                       // (alpha * acc)
    >;
...
template <
    ...
    // Fusion Op arguments
    bool IsOneShot, class SystemBarrier, class GmemLayoutTagD, class ElementD, class ElementCompute,
    class ElementC, class ElementScalar, FloatRoundStyle RoundStyle,
    // Epilogue arguments
...
struct FusionCallbacks<
    ...
    Sm90LinCombAuxAllReduce<IsOneShot, SystemBarrier, GmemLayoutTagD, ElementD, ElementCompute,
                            ElementC, ElementScalar, RoundStyle>,
    CtaTileShapeMNK, EpilogueTile, SmemLayoutAtom, CopyOpR2S>
    : Sm90LinearCombAuxAllReduce<StagesD, IsOneShot, SystemBarrier, GmemLayoutTagD, ElementD,
                                 ElementCompute, ElementC, ElementScalar, RoundStyle,
                                 CtaTileShapeMNK, EpilogueTile, SmemLayoutAtom, CopyOpR2S> {
  using Impl =
      Sm90LinearCombAuxAllReduce<StagesD, IsOneShot, SystemBarrier, GmemLayoutTagD, ElementD,
                                 ElementCompute, ElementC, ElementScalar, RoundStyle,
                                 CtaTileShapeMNK, EpilogueTile, SmemLayoutAtom, CopyOpR2S>;
  using Operation = Sm90LinCombAuxAllReduce<IsOneShot, SystemBarrier, GmemLayoutTagD, ElementD,
                                            ElementCompute, ElementC, ElementScalar, RoundStyle>;
