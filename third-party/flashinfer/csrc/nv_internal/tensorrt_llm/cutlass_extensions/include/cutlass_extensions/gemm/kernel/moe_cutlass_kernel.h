...
/////////////////////////////////////////////////////////////////////////////////////////////////
// This section exists to that we can use the same kernel code for regular gemm and dequantizing
// gemms. It will dispatch to the dequantizing gemm if the Mma type has an Iterator for scales in
// global.
template <typename...>
using void_t = void;
...
struct MoeFCGemm {
 public:
  ...
  struct Arguments {
    ...
    Arguments(int problem_count, int threadblock_count, int group_size,
              ...
              ElementB const* ptr_B, ElementScale const* weight_scales,
              ElementScale const* weight_zeros, ElementC const* ptr_C, bool C_is_broadcast,
              ElementC* ptr_D, int64_t const* total_tokens_including_expert, int64_t gemm_n,
 ...
 public:
  ...
  static Status can_implement(Arguments const& args) {
    if constexpr (platform::is_same<uint8_t, ElementB>::value ||
                  platform::is_same<uint4b_t, ElementB>::value) {
      ...
      CUTLASS_TRACE_HOST(
          "MoeFCGemm::can_implement() - weight scales are ignored for all types except uint8_t and "
          "uint4b_t");
      return Status::kInvalid;
  ...
  template <typename IteratorScale, WeightOnlyQuantOp op,
            std::enable_if_t<isFinegrained(op), bool> = true>
  CUTLASS_DEVICE static IteratorScale initialize_scale(
      typename IteratorScale::Params const& params, typename IteratorScale::Pointer pointer_scale,
      typename IteratorScale::Pointer pointer_zero, typename IteratorScale::TensorCoord extent,
      int thread_id, typename IteratorScale::TensorCoord const& threadblock_offset,
      int group_size) {
    return IteratorScale(params, pointer_scale, pointer_zero, extent, thread_id, threadblock_offset,
                         group_size);
  }
    ...
    while (problem_visitor.next_tile()) {
      ...
      ElementScale* ptr_Scale =
          use_dq_gemm<Mma>::value
              ...
              : nullptr;
      ElementScale* ptr_Zero =
          (params.weight_zeros == nullptr)
              ? nullptr
      ...
      typename Mma::IteratorB iterator_B(
          LayoutB(ldm_B), ptr_B, {problem_size.k() * kInterleave, problem_size.n() / kInterleave},
      ...
      if constexpr (use_dq_gemm<Mma>::value) {
        ...
        typename Mma::IteratorScale iterator_scale =
            initialize_scale<typename Mma::IteratorScale, Mma::QuantOp>(
                ...
                {scale_row_extent, problem_size.n()}, thread_idx, tb_offset_scale,
                params.group_size);
