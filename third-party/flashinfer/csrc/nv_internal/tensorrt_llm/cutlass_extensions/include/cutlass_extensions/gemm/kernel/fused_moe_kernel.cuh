...
namespace fused_moe {
template <typename ElementInput_, typename ElementWeight_, typename ElementOutput_, int MaxTileM_,
...
struct Fused_Moe_Kernel_sm80 {
  ...
  using BaseKernelTraits =
      Fused_Moe_Kernel_traits_sm80<ElementInput, ElementWeight, ElementOutput, kMaxTileM, kTileN,
  ...
  using ProblemVisitor = cutlass::gemm::kernel::MoeProblemVisitor<
      cutlass::gemm::kernel::detail::GemmGroupedProblemSizeHelper<
          cutlass::gemm::GemmShape<kMaxTileM, kTileN, kTileK>, false>,
      cutlass::gemm::GemmShape<kMaxTileM, kTileN, kTileK>,
      cutlass::gemm::kernel::GroupScheduleMode::kDeviceOnly, BaseKernelTraits::kThreadCount,
      BaseKernelTraits::kThreadCount>;
  ...
  using BaseKernelTraits_m16 =
      Fused_Moe_Kernel_traits_sm80<ElementInput, ElementWeight, ElementOutput, 16, kTileN, kTileK,
                                   kStages, activation_type>;
  static constexpr bool use_m16 =
      TileK_ >= 64;  // use tileshape m = 16 when original tileshape k >= 64

  static constexpr int kSmemSize =
      use_m16 ? (BaseKernelTraits::kSmemSize > BaseKernelTraits_m16::kSmemSize
  ...
  static Params to_underlying_arguments(Arguments const& args) {
    return {{args.routine_args.ptr_input, args.routine_args.ptr_fc1, args.routine_args.ptr_bias,
             ...
             args.routine_args.bias_is_broadcast},
            args.threadblock_count,
            {args.routine_args.total_tokens_including_expert, args.routine_args.gemm_n,
             args.routine_args.gemm_k, args.problem_count, nullptr, 0}};
  }
...
#define ROUTINE_PATH(kTileM_size)                                                                 \
  {                                                                                               \
    ...
    using RoutineTraits =                                                                         \
        Fused_Moe_Kernel_routine_sm80<ElementInput, ElementWeight, ElementOutput, kTileM, kTileN, \
                                      kTileK, kStages, activation_type>;                          \
    ...
    while (problem_visitor.next_tile()) {
      ...
      if (residue_m > kMaxTileM / 2) {
        using RoutineTraits =
            Fused_Moe_Kernel_routine_sm80<ElementInput, ElementWeight, ElementOutput, kMaxTileM,
                                          kTileN, kTileK, kStages, activation_type>;
        RoutineTraits routine{};
        routine.run_routine(params.routine_params, problem_index, block_m_idx_temp, block_n_idx,
                            gemm_m);
      } else {
...
static int fused_gemm_maximum_active_blocks(int smem_capacity = -1) {
  ...
  cudaError_t result;
  if (smem_size > (48 << 10)) {
    result = cudaFuncSetAttribute(run_global<GemmType>, cudaFuncAttributeMaxDynamicSharedMemorySize,
                                  smem_size);
  ...
  result = cudaOccupancyMaxActiveBlocksPerMultiprocessor(&max_active_blocks, run_global<GemmType>,
                                                         GemmType::kThreadCount, smem_size);
