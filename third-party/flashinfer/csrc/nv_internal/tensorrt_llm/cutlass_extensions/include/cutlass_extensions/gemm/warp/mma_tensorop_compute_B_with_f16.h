...
class MmaTensorOpComputeBWithF16 {
 ...
 public:
  ...
  using IteratorA =
      MmaTensorOpMultiplicandTileIterator<MatrixShape<Shape::kM, Shape::kK>, Operand::kA, ElementA,
                                          LayoutA,
  ...
  using IteratorB = MmaTensorOpMultiplicandTileIterator<
      ...
      MatrixShape<SharedMemoryInstructionShape::kK, InstructionShape::kN>, Policy::OpDelta::kRow,
  ...
  using IteratorC =
      MmaTensorOpAccumulatorTileIterator<MatrixShape<Shape::kM, Shape::kN>, ElementC, LayoutC,
 ...
 public:
  ...
  CUTLASS_DEVICE
  void operator()(FragmentC& D, TransformedFragmentA const& A, TransformedFragmentB const& B,
                  FragmentC const& C, int const warp_tileB_k_offset) const {
    ...
    static_assert(TransformedFragmentB::kElements ==
                  ...
                  "Each thread should have a pack of mma registers for each column iteration AND "
                  "for the expanded K dim of "
                  "B");
    ...
    for (int n = 0; n < MmaIterations::kColumn; ++n) {
      ...
      for (int m = 0; m < MmaIterations::kRow; ++m) {
        ...
        if (AccumulatorsInRowMajor) {  // matrix B is reordered
          mma(ptr_D[n + m_serpentine * MmaIterations::kColumn], ptr_A[m_serpentine],
              ptr_B[n_offsetB], ptr_D[n + m_serpentine * MmaIterations::kColumn]);
        } else {
    ...
    for (int m = 0; m < MmaIterations::kRow; ++m) {
      ...
      for (int n = 0; n < MmaIterations::kColumn; ++n) {
        ...
        if (AccumulatorsInRowMajor) {  // matrix B is reordered
          mma(ptr_D[n_serpentine + m * MmaIterations::kColumn], ptr_A[m],
              ptr_B[n_serpentine_offsetB], ptr_D[n_serpentine + m * MmaIterations::kColumn]);
        } else {
