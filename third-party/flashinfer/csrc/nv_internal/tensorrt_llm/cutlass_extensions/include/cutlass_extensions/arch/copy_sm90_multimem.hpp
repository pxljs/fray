...
struct SM90_MULTIMEM_LDREDUCE_F16x8 {
  ...
  CUTE_HOST_DEVICE static void copy(void const* gmem_mc_ptr, uint32_t& dst0, uint32_t& dst1,
...
struct SM90_MULTIMEM_ST_F16x8 {
  ...
  CUTE_HOST_DEVICE static void copy(void const* gmem_mc_ptr, uint32_t const& src0,
                                    uint32_t const& src1, uint32_t const& src2,
                                    uint32_t const& src3) {
#if defined(CUTE_ARCH_MULTIMEM_SM90_ENABLED)
    uint64_t gmem_addr = reinterpret_cast<uint64_t>(gmem_mc_ptr);
    // Load gmem_addr from each GPU, add srcX, and store back.
    asm volatile("multimem.st.global.v4.f16x2 [%0], {%1,%2,%3,%4};" ::"l"(gmem_addr), "r"(src0),
...
struct SM90_MULTIMEM_ST_BF16x8 {
  ...
  CUTE_HOST_DEVICE static void copy(void const* gmem_mc_ptr, uint32_t const& src0,
    ...
    uint64_t gmem_addr = reinterpret_cast<uint64_t>(gmem_mc_ptr);
    // Load gmem_addr from each GPU, add srcX, and store back.
    asm volatile("multimem.st.global.v4.bf16x2 [%0], {%1,%2,%3,%4};" ::"l"(gmem_addr), "r"(src0),
                 "r"(src1), "r"(src2), "r"(src3)
                 : "memory");
#else
    CUTE_INVALID_CONTROL_PATH("Trying to use multimem.red without CUTE_ARCH_NVLS_SM90_ENABLED.");
