...
__forceinline__ __device__ uint32_t atomicCAS_system_acq(uint32_t* p, uint32_t compare,
                                                         uint32_t val) {
  uint32_t result;
  asm volatile("atom.acquire.sys.global.cas.b32 %0, [%1], %2, %3;"
               : "=r"(result)
               : "l"(p), "r"(compare), "r"(val));
  return result;
}
...
struct MulticastSystemBarrier : public GenericBarrier<Sync> {
 ...
 protected:
  ...
  static void red_release(T* mc_ptr, int val) {
    ...
    // atomicAdd_system(arrival_counter_mc, 1); See
    ...
    asm volatile("multimem.red.release.sys.global.add.u32 [%0], %1;" ::"l"(mc_ptr), "r"(val)
                 : "memory");

    // Need a fence between MC and UC access to the same memory:
    // - fence.proxy instructions establish an ordering between memory accesses that may happen
    // through different proxies
    // - Value .alias of the .proxykind qualifier refers to memory accesses performed using
    // virtually aliased addresses to the same memory location. from
...
#pragma unroll 1
  ...
  CUTLASS_DEVICE
  static T arrive_inc_get(T* mc_ptr, T* uc_ptr, int thread_idx, int flag_idx, int rank,
                          int world_size) {
  ...
  CUTLASS_DEVICE
  static void arrive_inc(Params const& params, int thread_idx, int flag_idx, int rank,
                         int world_size) {
  ...
  CUTLASS_DEVICE
  static void arrive_and_wait(Params const& params, int thread_idx, int flag_idx, int rank,
                              int world_size) {
...
#pragma unroll 1
  ...
  CUTLASS_DEVICE
  static T arrive_inc_get(T** barrier_ptrs, int thread_idx, int flag_idx, int rank,
                          int world_size) {
    Sync::sync();
