 ...
 */
/*
  This file exists so that we use the same weight layout for MoE grouped gemm and regular gemm when
  the weight is quantized. The preprocessing code reads this template to know how to organize the
  quantized weight matrices to be consumed by CUTLASS.
...
struct LayoutDetailsB<TypeA, half_t, Arch,
...
template <typename TypeA, typename Arch>
struct LayoutDetailsB<TypeA, bfloat16_t, Arch,
                      typename platform::enable_if<Arch::kMinComputeCapability >= 75>::type> {
  static constexpr int ThreadblockK = 128 * 8 / cutlass::sizeof_bits<TypeA>::value;
  using Layout = layout::ColumnMajor;
...
// OpMultiplyAddDequantizeInterleavedBToA, which signals that we want to dequantize after loading
// from smem.
template <typename TypeA, typename Arch>
