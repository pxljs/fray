...
class GemmARUniversal<
    ...
    cute::enable_if_t<cute::is_base_of_v<KernelTmaWarpSpecialized,
 ...
 public:
  ...
  struct SharedStorage {
    // Mainloop and epilogue don't use smem concurrently since kernel is non-persistent, so we can
    // use a union
    union TensorStorage {
...
// Any Tensor Op MMA Atom in the WGMMA ISA is arch conditional to sm90a.
    ...
    enum class ProducerWarpRole { MainloopEpilogue = 0, Warp1 = 1, Warp2 = 2, Warp3 = 3 };
    ...
    if (warp_group_role == WarpGroupRole::Producer) {
      ...
      collective_mainloop.mma(mainloop_pipeline, mainloop_pipe_consumer_state, accumulators,
                              k_tile_count, warp_group_thread_idx, shared_storage.tensors.mainloop,
                              params.mainloop);
      ...
      constexpr uint32_t AllReduceBarrier =
