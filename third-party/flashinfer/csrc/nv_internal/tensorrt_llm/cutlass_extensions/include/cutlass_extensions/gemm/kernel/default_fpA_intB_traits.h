...
// ======================= Turing Traits ==============================
// Note that turing does not have native bfloat support so weights and activations will be casted to
// fp16 and compute will happen in fp16 then will be converted for bf16 output.
...
struct MixedGemmArchTraits<
 ...
 public:
  ...
  using InstructionShape =
      cutlass::gemm::GemmShape<16, 8, 256 / cutlass::sizeof_bits<TypeA>::value>;
...
struct MixedGemmArchTraits<
 ...
 public:
  ...
  using OperatorClass = cutlass::arch::OpClassTensorOp;
  using AccType = float;
  // be careful, TypeC should align with
  // TmaWarpSpecializedGroupedGemmInput::OutputTypeAdaptor_t<TypeA>
