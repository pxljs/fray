...
/*! \file
    \brief Templates for visiting scales to be used when dequantizing the weights for weight-only
   GEMM quantization.
*/
...
class FineGrainedScaleZeroIterator<Shape_, Element_, layout::RowMajor, 0, Alignment_> {
 ...
 public:
  ...
  FineGrainedScaleZeroIterator(
    ...
    const LongIndex tb_row_byte_offset = threadblock_offset.row() / (group_size / 64) *
                                         params_.stride_ * sizeof_bits<Element>::value / 8;
    ...
    // For the rows, we must check that we are within the extent AND the tile to avoid extra reads
    // on a given iteration. The same threads will be responsible for issues reads since the number
    // of scales read in a given iteration is a constant. Therefore, we should never have to update
  ...
  CUTLASS_HOST_DEVICE FineGrainedScaleZeroIterator(
      ...
      int group_size)
      : FineGrainedScaleZeroIterator(params, pointer_scale, pointer_zero, extent, thread_id,
                                     make_Coord(0, 0), group_size) {}
