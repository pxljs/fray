...
class DqMmaPipelined<Shape_, IteratorA_, SmemIteratorA_, IteratorB_, SmemIteratorB_, IteratorScale_,
 ...
 public:
  ...
  DqMmaPipelined(
      ...
      int const group_size,  ///< Will not be used, just to adapt to finegrained modifications and
                             ///< make the compilation successful. Because DqMmaPipelined is only
                             ///< enabled for sm<80, so even if this argument is not added, it does
                             ///< not affect compilation for sm>=80.
