...
class MmaTensorOpDequantizer<
 ...
 public:
  ...
  using FragmentScale =
      Array<ElementScale, kColsPerMmaPerThread * MmaOperator::MmaIterations::kColumn>;
  using FragmentZero =
      Array<ElementScale, kColsPerMmaPerThread * MmaOperator::MmaIterations::kColumn>;
  ...
  CUTLASS_DEVICE
  MmaTensorOpDequantizer(TensorRef smem_scales, TensorRef smem_zeros, int const warp_idx_n,
                         int const lane_idx) {
...
#else
    // Slow path not implemented here on purpose. If we need to do HMMA on older arch, scale
    // conversion should happen before scales are stored to shared memory and we should use the fp16
    // dequantizer. This will avoid numerous conversion instructions in GEMM main loop.
...
class MmaTensorOpDequantizer<
 ...
 public:
  ...
  void dequantize(FragmentDequantizedOperand& operand_frag, FragmentScale const& scale_frag) {
    using _MmaOperandB = typename ArchMmaOperator::FragmentB;
    using ExpandedMmaOperandB = Array<typename FragmentDequantizedOperand::Element,
  ...
  void dequantize(FragmentDequantizedOperand& operand_frag, FragmentScale const& scale_frag,
    ...
    if constexpr (hasZero(QuantOp)) {
      ...
      CUTLASS_PRAGMA_UNROLL
      for (int mma_n_iter = 0; mma_n_iter < MmaOperator::MmaIterations::kColumn; ++mma_n_iter) {
        operand_frag_ptr[mma_n_iter] = plus_op(
            mul_op(operand_frag_ptr[mma_n_iter], scale_frag[mma_n_iter]), zero_frag[mma_n_iter]);
      }
    } else {
      CUTLASS_PRAGMA_UNROLL
      for (int mma_n_iter = 0; mma_n_iter < MmaOperator::MmaIterations::kColumn; ++mma_n_iter) {
