...
cudaError_t cudaMemcpyAsyncSanitized(void* dst, void const* src, size_t count,
                                     enum cudaMemcpyKind kind, cudaStream_t stream = nullptr);
...
#ifdef ENABLE_FP8
void invokeCudaD2Dcpyfp82Float(float* dst, __nv_fp8_e4m3* src, size_t const size,
                               cudaStream_t stream);
void invokeCudaD2Dcpyfp82Half(half* dst, __nv_fp8_e4m3* src, size_t const size,
                              cudaStream_t stream);
void invokeCudaD2DcpyFloat2fp8(__nv_fp8_e4m3* dst, float* src, size_t const size,
                               cudaStream_t stream);
void invokeCudaD2DcpyHalf2fp8(__nv_fp8_e4m3* dst, half* src, size_t const size,
                              cudaStream_t stream);
void invokeCudaD2DcpyBfloat2fp8(__nv_fp8_e4m3* dst, __nv_bfloat16* src, size_t const size,
                                cudaStream_t stream);
#endif  // ENABLE_FP8
#ifdef ENABLE_BF16
void invokeCudaD2DcpyBfloat2Float(float* dst, __nv_bfloat16* src, size_t const size,
                                  cudaStream_t stream);
#endif  // ENABLE_BF16
...
__inline__ __host__ __device__ std::enable_if_t<
    std::is_pointer<TDim>::value,
    typename std::remove_pointer<TDim>::type> constexpr flat_index(TDim dims, TIndex const& index,
                                                                   TIndices... indices) {
  assert(index < dims[0]);
  return flat_index(static_cast<typename std::remove_pointer<TDim>::type>(index), dims + 1,
                    indices...);
}

template <unsigned skip = 0, typename T, std::size_t N, typename TIndex, typename... TIndices>
__inline__ __host__ __device__ T constexpr flat_index(std::array<T, N> const& dims,
...
template <unsigned skip = 0, typename T, typename TIndex, std::size_t N, typename... TIndices>
__inline__ __host__ __device__ T constexpr flat_index(T const (&dims)[N], TIndex const& index,
                                                      TIndices... indices) {
  static_assert(skip < N);
  static_assert(sizeof...(TIndices) < N - skip, "Number of indices exceeds number of dimensions");
...
__inline__ __host__ __device__ T constexpr flat_index2(TIndex const& index_0, TIndex const& index_1,
                                                       T const& dim_1) {
...
template <typename T, typename TIndex>
__inline__ __host__ __device__ T constexpr flat_index3(TIndex const& index_0, TIndex const& index_1,
                                                       TIndex const& index_2, T const& dim_1,
                                                       T const& dim_2) {
...
__inline__ __host__ __device__ T constexpr flat_index4(TIndex const& index_0, TIndex const& index_1,
                                                       TIndex const& index_2, TIndex const& index_3,
...
__inline__ __host__ __device__ T constexpr flat_index5(TIndex const& index_0, TIndex const& index_1,
                                                       TIndex const& index_2, TIndex const& index_3,
                                                       TIndex const& index_4, T const& dim_1,
                                                       T const& dim_2, T const& dim_3,
                                                       T const& dim_4) {
...
template <typename T, typename TIndex>
__inline__ __host__ __device__ T constexpr flat_index_strided3(TIndex const& index_0,
...
__inline__ __host__ __device__ T constexpr flat_index_strided4(
    TIndex const& index_0, TIndex const& index_1, TIndex const& index_2, TIndex const& index_3,
    T const& stride_1, T const& stride_2, T const& stride_3) {
  assert(index_1 < stride_1 / stride_2);
...
template <typename T>
void invokeInPlaceTranspose0213(T* data, T* workspace, size_t const dim0, size_t const dim1,
                                size_t const dim2, size_t const dim3);

template <typename T>
void invokeInPlaceTranspose102(T* data, T* workspace, size_t const dim0, size_t const dim1,
                               size_t const dim2);
...
void invokeCudaD2DcpyConvert(T_OUT* tgt, const T_IN* src, size_t const size,
                             cudaStream_t stream = 0);

template <typename T_IN, typename T_OUT>
void invokeCudaD2DScaleCpyConvert(T_OUT* tgt, const T_IN* src, float const* scale,
                                  bool invert_scale, size_t const size, cudaStream_t stream = 0);
...
template <typename T>
bool invokeCheckRange(T const* buffer, size_t const size, T min, T max, bool* d_within_range,
                      cudaStream_t stream);
...
AlignedPointersUnpacker inline calcAlignedPointers(void const* p, std::vector<size_t> const& sizes,
                                                   size_t ALIGN_BYTES = DEFAULT_ALIGN_BYTES) {
