...
bool is_valid_split_k_factor(int64_t const m, int64_t const n, int64_t const k,
                             TileShape const tile_shape, int const split_k_factor,
                             size_t const workspace_bytes, bool const is_weight_only) {
...
std::vector<CutlassTileConfig> get_candidate_tiles(
  ...
  enum class CutlassGemmType : char { Default, WeightOnly, Simt, Int8, Fp8 };
...
// We only compile CUTLASS kernels with multi-cast along M if the M tile is >= 128. This is purely
...
// We only compile CUTLASS kernels with multi-cast along N if the N tile is >= 128. This is purely
// to improve compilation speed.
...
std::vector<CutlassGemmConfig> get_candidate_configs_sm90(
  ...
  for (auto const& tile_config : tiles) {
    ...
    if (has_w4afp8) {
      ...
      if (has_m_mcast && has_n_mcast) {
        CutlassGemmConfig candidate(tile_config, MainloopScheduleType::AUTO,
                                    EpilogueScheduleType::AUTO, ClusterShape::ClusterShape_2x2x1);
        candidate_configs.push_back(candidate);
      }
    }
  }
  ...
  if (config & CutlassGemmConfig::WEIGHT_ONLY) {
    if (tiles.size() > 0 && !(config & CutlassGemmConfig::GROUPED_GEMM)) {
      CutlassGemmConfig CudaKernelConfig(tiles[0], MainloopScheduleType::AUTO,
  ...
  if (config & CutlassGemmConfig::GROUPED_GEMM) {
    ...
    if ((config & CutlassGemmConfig::FP4_ONLY) != 0) {
      candidate_configs.push_back(CutlassGemmConfig{
          CutlassTileConfigSM100::CtaShape128x128x128B, MainloopScheduleType::AUTO,
          EpilogueScheduleType::AUTO, ClusterShape::ClusterShape_1x1x1});
      candidate_configs.push_back(CutlassGemmConfig{
      ...
      candidate_configs.push_back(
          CutlassGemmConfig{CutlassTileConfigSM100::CtaShape256x64x128B, MainloopScheduleType::AUTO,
                            EpilogueScheduleType::AUTO, ClusterShape::ClusterShape_2x1x1});
      candidate_configs.push_back(
          CutlassGemmConfig{CutlassTileConfigSM100::CtaShape128x64x128B, MainloopScheduleType::AUTO,
                            EpilogueScheduleType::AUTO, ClusterShape::ClusterShape_1x1x1});
      return candidate_configs;
    }
...
CutlassGemmConfig estimate_best_config_from_occupancies(
    std::vector<CutlassGemmConfig> const& candidate_configs, std::vector<int> const& occupancies,
    int64_t const m, int64_t const n, int64_t const k, int64_t const num_experts,
    int const split_k_limit, size_t const workspace_bytes, int const multi_processor_count,
  ...
  for (int ii = 0; ii < candidate_configs.size(); ++ii) {
    ...
    for (int split_k_factor = 1; split_k_factor <= max_split_k; ++split_k_factor) {
      if (is_valid_split_k_factor(m, n, k, tile_shape, split_k_factor, workspace_bytes,
                                  is_weight_only)) {
        ...
        if (current_score < config_score ||
          ...
          best_config = CutlassGemmConfig(candidate_config.tile_config_sm80, split_style,
                                          split_k_factor, candidate_config.stages);
          current_m_tile = tile_shape.m;
        } else if (current_score == config_score &&
                   (best_config.stages < candidate_config.stages ||
                    split_k_factor < best_config.split_k_factor || current_m_tile < tile_shape.m)) {
