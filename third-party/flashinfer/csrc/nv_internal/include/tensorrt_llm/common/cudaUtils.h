...
template <typename T>
void checkEx(T ptr, std::initializer_list<T> const& validReturns, char const* const func,
...
inline std::tuple<size_t, size_t> getDeviceMemoryInfo(bool const useUvm) {
  if (useUvm) {
    ...
    TLLM_LOG_INFO(
        "Using UVM based system memory for KV cache, total memory %0.2f GB, available memory %0.2f "
        "GB",
...
///
/// @return size_t The size of the smallest difference in memory size supported by the current
/// device.
...
inline int getMaxSharedMemoryPerSM() {
  ...
  check_cuda_error(cudaDeviceGetAttribute(&nByteMaxSharedMemoryPerSM,
                                          cudaDevAttrMaxSharedMemoryPerMultiprocessor, deviceID));
...
inline int getMaxSharedMemoryPerBlockOptin() {
  ...
  int deviceID{0};
  check_cuda_error(cudaGetDevice(&deviceID));
  check_cuda_error(cudaDeviceGetAttribute(&nByteMaxSharedMemoryPerBlockOptin,
...
template <typename T>
void printArrayInfo(T const* ptr, uint64_t nElement = 1, std::string name = "",
                    bool const bPrintElement = false) {
  if (ptr == nullptr) {
  ...
  TLLM_LOG_INFO("avg=%f, absSum: %f, std=%f, max=%f, min=%f, sad=%f", avg, absSum, std, allMax,
                allMin, allSad);
...
template void printArrayInfo(float const* ptr, uint64_t nElement, std::string name,
                             bool const bPrintElement);
template void printArrayInfo(half const* ptr, uint64_t nElement, std::string name,
                             bool const bPrintElement);
#ifdef ENABLE_BF16
template void printArrayInfo(__nv_bfloat16 const* ptr, uint64_t nElement, std::string name,
                             bool const bPrintElement);
#endif
#ifdef ENABLE_FP8
template void printArrayInfo(__nv_fp8_e4m3 const* ptr, uint64_t nElement, std::string name,
                             bool const bPrintElement);
#endif
template void printArrayInfo(uint32_t const* ptr, uint64_t nElement, std::string name,
                             bool const bPrintElement);
template void printArrayInfo(uint64_t const* ptr, uint64_t nElement, std::string name,
                             bool const bPrintElement);
template void printArrayInfo(int const* ptr, uint64_t nElement, std::string name,
                             bool const bPrintElement);
template void printArrayInfo(uint8_t const* ptr, uint64_t nElement, std::string name,
                             bool const bPrintElement);
...
template <typename T>
void print2dToFile(std::string fname, T const* ptr, int const nRow, int const nCol,
                   int const nStride) {
...
__host__ __device__ inline void print_float_(float x) { printf("%7.3f ", x); }

__host__ __device__ inline void print_element_(float x) { print_float_(x); }
...
__host__ __device__ inline void print_element_(uint32_t ul) { printf("%7" PRIu32 " ", ul); }

__host__ __device__ inline void print_element_(uint64_t ull) { printf("%7" PRIu64 " ", ull); }

__host__ __device__ inline void print_element_(int32_t il) { printf("%7" PRId32 " ", il); }

__host__ __device__ inline void print_element_(int64_t ill) { printf("%7" PRId64 " ", ill); }
...
#ifdef ENABLE_BF16
template __device__ void printMatrixDevice(__nv_bfloat16 const* ptr, int nRow, int nCol,
                                           int nStride);
#endif
#ifdef ENABLE_FP8
template __device__ void printMatrixDevice(__nv_fp8_e4m3 const* ptr, int nRow, int nCol,
                                           int nStride);
#endif
...
__forceinline__ __device__ void issue_stas(uint32_t dist_barrier_ptr, uint32_t dist_buffer_ptr,
                                           uint32_t d0) {
...
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900) && (__CUDACC_VER_MAJOR__ >= 12))
  asm volatile(
      "st.async.weak.shared::cluster.mbarrier::complete_tx::bytes.v2.b32 [%0], {%2, %3}, [%1];\n\t"
      :
...
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 900) && (__CUDACC_VER_MAJOR__ >= 12))
  asm volatile(
      "st.async.shared::cluster.mbarrier::complete_tx::bytes.v4.b32 [%0], {%2, %3, %4, %5}, "
      "[%1];\n\t"
      :
...
struct Arrive_wait {
 public:
  __forceinline__ __device__ Arrive_wait() { bar_base_ = NULL; }
  ...
  __forceinline__ __device__ uint64_t* bar_base() { return bar_base_; }

  __forceinline__ __device__ uint64_t* get_bar_addr(int id) { return bar_base_ + id; }
...
__forceinline__ __device__ T* get_DSMEM_ptr(T* localAddress, uint32_t destCtaId) {
  T* dsmemAddress = reinterpret_cast<T*>(
      ((unsigned long long int)localAddress & 0xFFFFFFFFF0FFFFFFULL) + (destCtaId << 24));
  return dsmemAddress;
}
...
__forceinline__ __device__ void arrive_DSMEM_barrier_and_set_tx_cnt(uint64_t* p_barrier,
                                                                    uint32_t ctaid,
                                                                    uint32_t expected_copy_bytes) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800))
  uint32_t p_bar =
      static_cast<uint32_t>(reinterpret_cast<uintptr_t>(get_DSMEM_ptr(p_barrier, ctaid)));
  asm volatile("{mbarrier.arrive.expect_tx.b64 _, [%0], %1; \n\t}" ::"r"(p_bar),
               "r"(expected_copy_bytes));
#endif
}

template <bool barSetTxCnt = true>
__forceinline__ __device__ void stas(uint32_t* p_data, uint64_t* p_barrier, uint32_t ctaid,
                                     uint32_t const& wrdat) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800))
  if (barSetTxCnt) arrive_DSMEM_barrier_and_set_tx_cnt(p_barrier, ctaid, sizeof(uint32_t));
...
template <bool barSetTxCnt = true>
__forceinline__ __device__ void stas(uint64_t* p_data, uint64_t* p_barrier, uint32_t ctaid,
                                     uint64_t const& wrdat) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800))
  if (barSetTxCnt) arrive_DSMEM_barrier_and_set_tx_cnt(p_barrier, ctaid, sizeof(uint64_t));
  uint32_t buffer_ptr = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(p_data));
  uint32_t barrier_ptr = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(p_barrier));
...
template <bool barSetTxCnt = true>
__forceinline__ __device__ void stas(uint64_t* p_data, uint64_t* p_barrier, uint32_t ctaid,
                                     uint32_t const wrdat0, uint32_t const wrdat1,
                                     uint32_t const wrdat2, uint32_t const wrdat3) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800))
  if (barSetTxCnt) arrive_DSMEM_barrier_and_set_tx_cnt(p_barrier, ctaid, 4 * sizeof(uint32_t));
  uint32_t buffer_ptr = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(p_data));
  uint32_t barrier_ptr = static_cast<uint32_t>(reinterpret_cast<uintptr_t>(p_barrier));
...
__forceinline__ __device__ void stas(T* p_data, uint64_t* p_barrier, uint32_t ctaid,
                                     T const& wrdat) {
#if (defined(__CUDA_ARCH__) && (__CUDA_ARCH__ >= 800))
  static_assert(sizeof(T) % 4 == 0);
  if (barSetTxCnt) arrive_DSMEM_barrier_and_set_tx_cnt(p_barrier, ctaid, sizeof(T));
  ...
  for (uint32_t offset = 0; offset < sizeof(T);) {
    if constexpr (assumeAligned) {
      if (offset + 16 <= sizeof(T)) {
        ...
        issue_stas(buffer_ptr_ + offset, barrier_ptr_, p_wrdat_b32[offset / 4],
                   p_wrdat_b32[offset / 4 + 1], p_wrdat_b32[offset / 4 + 2],
                   p_wrdat_b32[offset / 4 + 3]);
        offset += 16;
...
class OrderedMutexAccessor {
 ...
 public:
  __device__ OrderedMutexAccessor(OrderedMutex& m, int id, State state)
      : _phase(state.phase), _id(id), _barriers(m.barriers) {}

  __device__ void arrive() { _barriers.bar_arrive_normal(_id); }
...
// We use singleton memory pool and the order of destructors depends on the compiler implementation.
// We find that the cudaFree/cudaFreeHost is called after cudaruntime destruction on Windows. There
// will be an cudaErrorCudartUnloading error.  However, it is safe to ignore this error because the
// cuda runtime is already exited, we are no more worried about the memory leaks.
#define TLLM_CUDA_CHECK_FREE_RESOURCE(stat)                                               \
  do {                                                                                    \
    tensorrt_llm::common::checkEx((stat), {cudaSuccess, cudaErrorCudartUnloading}, #stat, \
                                  __FILE__, __LINE__);                                    \
  } while (0)
